...
events.get('/events', wrap(async (req, res) => {
  const rows = await prisma.event.findMany({ where: { status: { in: ['OPEN', 'CLOSED', 'FINISHED'] } }, orderBy: { startsAt: 'asc' }, include: { course: { select: { name: true, distanceM: true } }, _count: { select: { registrations: true } } } });
...
events.get('/events/:id', wrap(async (req, res) => {
  ...
  const ev = await prisma.event.findFirst({ where: { OR: [{ id }, { slug: id }] }, include: { course: true, _count: { select: { registrations: true } } } });
...
events.post('/events/:id/register', requireUser, wrap(async (req, res) => {
  ...
  const ev = await prisma.event.findUnique({ where: { id: String(req.params.id) }, include: { _count: { select: { registrations: true } } } });
  ...
  const reg = await prisma.eventRegistration.upsert({ where: { eventId_userId: { eventId: ev.id, userId: req.user!.id } }, create: { eventId: ev.id, userId: req.user!.id, tshirtSize: body.tshirtSize, bib: ev._count.registrations + 1, paid: ev.feeKrw === 0 }, update: { tshirtSize: body.tshirtSize } });
