...
async function request<T>(method: string, path: string, body?: unknown): Promise<T> {
  const res = await fetch(`${API_URL}/api${path}`, { method, headers: { 'content-type': 'application/json', 'x-device-id': deviceId() }, body: body === undefined ? undefined : JSON.stringify(body) });
