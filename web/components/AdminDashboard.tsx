...
export default function AdminDashboard() {
  ...
  function payload() {
    ...
    return { nickname: item.nickname, isActive: Boolean(item.isActive) };
  }
  ...
  async function logout() { await adminRequest('/auth/logout', { method: 'POST' }).catch(() => undefined); router.replace('/admin/login'); }

  return <main className="admin-page">
    <header className="admin-topbar"><Link href="/" className="admin-logo"><img src="/logo-white.png" alt="LOCAL STRIDE" /><span>ADMIN</span></Link><div><span>{user?.email}</span><Link href="/" className="admin-top-link">서비스 보기</Link><button type="button" onClick={logout}>로그아웃</button></div></header>
...
function DataList({ panel, rows, loading, onEdit, onDelete }: { panel: Panel; rows: Row[]; loading: boolean; onEdit: (row: Row) => void; onDelete: (row: Row) => void }) {
  ...
  return <div className="admin-data-list">{rows.map((row) => {
    const image = row.imageUrl || row.thumbnailUrl || row.firstImage; const title = row.title || row.name || row.nickname;
