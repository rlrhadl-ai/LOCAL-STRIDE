...
export default function AdminAuthForm({ mode }: { mode: 'login' | 'signup' }) {
  ...
  return <main className="admin-auth-page">
    <section className="admin-auth-card">
      ...
      <div className="admin-auth-copy"><span>ADMIN CONSOLE</span><h1>{signup ? '관리자 계정 설정' : '관리자 로그인'}</h1><p>{signup ? '기존 개인 계정이 있다면 같은 비밀번호와 최초 가입 코드를 입력하세요. 하나의 계정으로 서비스와 관리 화면을 함께 사용합니다.' : '콘텐츠와 서비스 운영 데이터를 안전하게 관리하세요.'}</p></div>
      <form onSubmit={submit} className="admin-auth-form">
        ...
        <label className="field">비밀번호<input className="input" type="password" value={password} onChange={(event) => setPassword(event.target.value)} minLength={8} maxLength={100} autoComplete={signup ? 'new-password' : 'current-password'} placeholder="영문과 숫자를 포함해 8자 이상" required /></label>
        {error && <p className="admin-error" role="alert">{error}</p>}
        <button className="btn" disabled={loading}>{loading ? '처리 중…' : signup ? '관리자 회원가입' : '로그인'}</button>
      </form>
      <p className="admin-auth-switch">{signup ? '이미 관리자 설정을 마쳤나요?' : '최초 접속인가요?'} <Link href={signup ? '/admin/login' : '/admin/signup'}>{signup ? '일반 계정으로 로그인' : '관리자 계정 설정'}</Link></p>
    </section>
  </main>;
}
