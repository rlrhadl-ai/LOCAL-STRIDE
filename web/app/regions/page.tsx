...
export default function RegionsPage() {
  ...
  const coverage = useMemo(() => DAEGU_AREAS.map((area) => {
    ...
    const courseCount = courses.filter((course) => course.slug.includes('modern-alley') ? area.name === '중구' : matches(`${course.areaName} ${course.name} ${course.description}`)).length;
    ...
    return { area, courseCount, communityCount, scheduleCount, active: courseCount + communityCount + scheduleCount > 0 };
  ...
  return <main className="page region-page">
    ...
    <section className="region-hero"><span>ALL AROUND DAEGU</span><h1>수성못에서 시작해<br />대구 전체로.</h1><p>9개 구·군의 러닝 거점을 선택하면 주변 관광지·커뮤니티·일정을 한번에 탐색할 수 있어요.</p><div><span><b>9</b>개 구·군</span><span><b>{loading ? '-' : activeCount}</b>현재 커뮤니티 데이터</span><span><b>5km</b>TourAPI 탐색 반경</span></div></section>
    ...
    <section className="region-grid">{coverage.map(({ area, courseCount, communityCount, scheduleCount, active }, index) => <article className={`region-card ${active ? 'active' : 'planned'}`} key={area.slug}>
      <div className="region-card-image"><img src={area.image} alt="" /><span>0{index + 1}</span><em>{active ? '탐색 가능' : '코스 검증 준비'}</em></div>
      <div className="region-card-copy"><small>{area.fullName} · {area.hub}</small><h2>{area.name}</h2><p>{area.summary}</p><div className="region-theme-row">{area.themes.map((theme) => <span key={theme}>{theme}</span>)}</div><dl><div><dt>검증 코스</dt><dd>{courseCount}</dd></div><div><dt>크루·메이트</dt><dd>{communityCount}</dd></div><div><dt>프로그램·대회</dt><dd>{scheduleCount}</dd></div></dl><div className="region-actions"><Link href={`/spots?area=${area.slug}`}>주변 장소</Link><Link href={`/mates?area=${area.slug}`}>러너 찾기</Link></div></div>
    </article>)}</section>
    <p className="region-disclaimer">‘탐색 가능’은 현재 시범 커뮤니티나 일정이 등록된 지역입니다. ‘코스 검증 준비’ 지역도 TourAPI 주변 정보는 탐색할 수 있지만, 러닝 경로는 보행 안전·통행·공사 정보를 확인한 뒤 공개합니다.</p>
  </main>;
}
