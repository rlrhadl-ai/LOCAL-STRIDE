...
export default function SpotsPage() {
  ...
  useEffect(() => {
    ...
    else { try { const saved = localStorage.getItem('localstride_daegu_area'); if (saved) setAreaSlug(daeguAreaByName(saved).slug); } catch { /* use default */ } }
  }, []);
  useEffect(() => {
    let alive = true; setLoading(true); setItems([]); setResult(null);
    const selected = daeguAreaByName(areaSlug);
    try { localStorage.setItem('localstride_daegu_area', selected.slug); } catch { /* storage is optional */ }
    window.history.replaceState(null, '', `/spots?area=${selected.slug}`);
    api.get<NearbyResult>(`/tour/nearby?lat=${selected.lat}&lng=${selected.lng}&radius=5000&limit=20`).then((next) => {
  ...
  return <main className="page local-content-page">
    ...
    <section className="area-switcher"><div><span>EXPLORE AREA</span><strong>{area.fullName}</strong><small>{area.hub} 기준 반경 5km</small></div><select aria-label="대구 지역 선택" value={areaSlug} onChange={(event) => setAreaSlug(event.target.value)}>{DAEGU_AREAS.map((option) => <option value={option.slug} key={option.slug}>{option.name} · {option.hub}</option>)}</select></section>
